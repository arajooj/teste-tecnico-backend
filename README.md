...
- **PostgreSQL — erro de tamanho de `version_num` ao rodar migrations:** em bancos antigos, a tabela `alembic_version` pode ter `version_num` com limite de 32 caracteres. Uma migration `0000` já amplia isso em novos bancos. Se você ainda encontrar erro de tamanho (ex.: revision longa), rode uma vez no Postgres: `ALTER TABLE alembic_version ALTER COLUMN version_num TYPE VARCHAR(64);` e depois `python -m alembic upgrade head`.
